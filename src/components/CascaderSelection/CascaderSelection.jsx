...
class CascadeSelection extends React.Component{
    ...
    render(){
        let selectedOptionLabel=this.state.selectedData?this.state.selectedData.label:'全部类目';
        ...
        return(
            <div className='cascade-wrap'>
                <div className='cascade-control' onClick={this.showPanel}>
                    <span className='select-arrow icon-dowico'></span>
                    <span className='selected-label' title={selectedOptionLabel}>{selectedOptionLabel}</span>
                </div>
                <CascadePanel optionData={this.state.optionData}  closePanel={this.closePanel} cascadePanelShow={this.state.cascadePanelShow} updateSelectedData={this.selectData}/>
            </div>
        );
    }
}
class CascadePanel extends  React.Component{
    ...
    updateChildren(item){
        ...
        existSelectedCategoryArr.forEach((ele,index)=>{//去掉同一级的选中类
        ...
        let selectedValue='';
        let selectedLabel='';
        if(item.children){//存在下级类目时，更新显示的类别项
            this.setState({
            ...
            newSelectedCategoryArr.forEach((ele)=>{//newSelectedCategoryArr
    ...
    traversalAllOptionData(arr,data){
       try{//解决forEach()无法使用break、continute\retutn false跳出循环的问题
        arr.forEach(item => {
    ...
    render(){
        ...
        return(
            <div className={cascadePanelCls}  >
                ...
                <span className='close-panel icon-close' onClick={this.closePanel}></span>
                <div className='option-title'>
                    
                    <div className='option-class' onClick={this.selecteAll}>全部类目<span className='icon-dowico'></span></div>
                    
                    {this.state.selectedCategoryArr.map((item)=>{
                        return(
                        <div className='option-class' key={item.label} onClick={this.tabOptionClass.bind(this,item)}>{item.label}<span className='icon-dowico'></span></div>
                        );
                    })}
                    <div className='option-class choose'>请选择<span className='icon-dowico'></span></div>
                </div>
                <ul className='option-panel'>
                    <li className='option' onClick={this.selectCurrentOptionClass}>全部</li>
                    {this.state.optionData.map((item)=>{
                        return (
                            <li className="option" key={item.value}  onClick={this.updateChildren.bind(this,item)}>{item.label}</li>
                        );
                    })}
                </ul>
            </div>
